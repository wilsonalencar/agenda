...
class Generate_Cronograma_Activities_Single extends Command
{
    protected $eService;
    /**
     ...
     * @var string
     */
    protected $signature = 'generatecronograma:single {cnpj} {codigo} {tributo_id} {periodo_ini} {periodo_fin?}';
            ...
            if (sizeof($period_array)==0) {
                ...
                for ($i = 0; $i<sizeof($period_array); $i++) {
                    ...
                    if (strlen($periodo) == 6 && substr($cnpj, -6, 4) != '0001' && $is_valid && is_numeric($tributo_id) && $tributo_id<20) {
                        ...
                        $retval = $this->eService->generateSingleCnpjCronActivities($periodo, $cnpj, $codigo, $tributo_id);
