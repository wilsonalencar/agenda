...
<div style="float:left; width:60%">
    <h2>ATIVIDADE</h2><small>(REF #{{ $atividade->id }})</small>
    ...
    <p class="lead">Status: <b>{{ status_label($atividade->status) }}</b>
    ...
    <p class="lead">Data entrega: <b>{{ substr($atividade->data_entrega,0,10) }}</b></p>
        @if ($atividade->status >2)
            <p class="lead">Data aprovação: <b>{{ substr($atividade->data_aprovacao,0,10) }}</b></p>
        @endif
    @else
    <p class="lead">Data limite entrega: <b>{{ substr($atividade->limite,0,10) }}</b></p>
    @endif

    @if ($atividade->obs != null)
    <p class="lead">Comentário: <b>{{ $atividade->obs }}</b></p>
    @endif
