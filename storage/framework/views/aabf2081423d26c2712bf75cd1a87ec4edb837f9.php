...
$(function() {
    $('#entregas-table').DataTable({
        ...
        stateSave: true,
        ajax: {
                    url: "<?php echo route('entregas.data'); ?>",
                    data: function (d) {
        ...
        columns: [
            ...
            {data: 'id', name:'edit', searchable: false, orderable: false, render: function (data, type, row) {
                                                    ...
                                                    switch(row['status']) {
                                                        case 1:
                                                                url = '<a href="<?php echo e(route('upload.entrega', ':id_atividade')); ?>" style="margin-left:10px" class="btn btn-default btn-sm">Entregar</a>';
                                                                url = url.replace(':id_atividade', data);
                                                                break;

                                                        case 2:
                                                                <?php if( Auth::user()->hasRole('admin') || Auth::user()->hasRole('owner') || Auth::user()->hasRole('supervisor')): ?>
                                                                url = '<a href="<?php echo e(route('entregas.show', ':id_atividade')); ?>" style="margin-left:10px" class="btn btn-danger btn-default btn-sm">Em aprovação</a>';
                                                                url = url.replace(':id_atividade', data);
                                                                <?php else: ?>
                                                                ...
                                                                break;
                                                        case 3:
                                                                url = '<a href="<?php echo e(route('entregas.show', ':id_atividade')); ?>" style="margin-left:10px" class="btn btn-success btn-default btn-sm">Recibo</a>';
                                                                url = url.replace(':id_atividade', data);
                                                                break;
                                                    }
                                                    return url;
    ...
    $('#adv_search').on('click', function(e) {
            ...
            if (val_cnpj || val_codigo || val_status) {
                var url = "<?php echo e(route('entregas.index')); ?>?vcn="+val_cnpj.replace(/[^0-9]/g,'')+"&vco="+val_codigo.replace(/[^0-9]/g,'')+"&vst="+val_status;
            } else {
                var url = "<?php echo e(route('entregas.index')); ?>";
            }
