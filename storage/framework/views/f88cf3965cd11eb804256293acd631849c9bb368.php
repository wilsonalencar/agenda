...
<p class="lead">Segue consulta de cadastros realizados. <a href="<?php echo e(route('processosadms.create')); ?>">Adicionar outro?</a></p>
<hr>
...
$(function() {
    ...
    $('#processosadms-table').DataTable({
        ...
        stateSave: true,
        ajax: {
                url: "<?php echo route('processosadms.data'); ?>",
                data: function (d) {
                    d.area = $('#src_area').val();
                    d.cnpj = $('#src_cnpj').val();
        ...
        columns: [
            ...
            {data: 'id', name:'edit', searchable: false, orderable: false, render: function (data) {
                ...
                url += '<a href="<?php echo e(route('processosadms.edit', ':id_edit')); ?>" class="btn btn-default btn-sm">Editar</a>';
                url += '<a href="<?php echo e(route('processosadms.delete', ':id_show')); ?>" style="margin-left:10px" class="btn btn-default btn-sm">X</a>';
    ...
    $('#adv_search').on('click', function(e) {
        ...
        if (val_cnpj || val_area || val_periodo) {
            var url = "<?php echo e(route('processosadms.search')); ?>?vcn="+val_cnpj.replace(/[^0-9]/g,'')+"&vco="+val_area+"&vcp="+val_periodo;
        } else {
            var url = "<?php echo e(route('processosadms.search')); ?>?clear=s";
        }
...
{   
    ...
    {
        type: "GET",
        url: '<?php echo e(url('processosadms')); ?>/search_observacao',
        cache: false,
        async: false,
        dataType: "json",
        data:
